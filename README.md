...
XASS объединяет персонального Telegram Business‑бота, мониторинг компьютеров и сервера, удалённые команды, управление публичным профилем, музыку, погоду, SwiftUI-приложение для iPhone и отдельный устанавливаемый PWA.
...
<p align="center">
  ...
  <img src="https://github.com/lucifervalter-a11y/XASS/releases/download/ios-latest/iOS-native-player.png" alt="Нативный плеер XASS с выбором устройства" width="280">
  <img src="https://github.com/lucifervalter-a11y/XASS/releases/download/ios-latest/iOS-native-devices.png" alt="Управление компьютерами в приложении iPhone" width="280">
  <br><sub>Реальные снимки приложения из iOS Simulator. Демонстрационные данные и явно маркированная тестовая обложка; не личная библиотека владельца. Основные экраны не используют WebView.</sub>
</p>
...
- Удаление привязки прямо в карточке агента, в том числе offline: ключ отзывается, ожидающие команды отменяются. Архивы и файлы не стираются.
...
- VK-подключение проверяет сохранённый токен и доступные права. **Массовый экспорт аудиотеки VK пока не подключён**: имеющийся доступ к статусу не даёт доступ к музыкальным файлам. XASS не запрашивает пароль VK, браузерные cookies или обход DRM.
- До 128 МиБ на файл по умолчанию; возобновляемые блоки по 512 КиБ, контроль свободного места и фактического аудиоформата. Удаление из библиотеки мягкое: байты сохраняются на сервере; временные незавершённые загрузки истекают через сутки.
