...
Если у вас домен сайта (например `https://redvps.site`), можно указывать его:
клиент попробует авто-определить backend по `/health` и портам `8001/8000`.
Если backend недоступен по этому URL, будет понятная ошибка (а не падение JSONDecodeError).
