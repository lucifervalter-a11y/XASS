...
from app.services.server_migration import _env_text, _no_symlinks, _prepare_sqlite, _restore_postgres, _safe_relative
...
def create_snapshot(root: Path, settings: Settings, destination: Path, *, password: str = "",
    ...
    # Detect values which dotenv would silently interpolate after restoration.
    _env_text(settings.model_dump(mode="json"))
    root = root.resolve()
    ...
    configured_paths = {resolved(root, getattr(settings, field)) for field in PATH_FIELDS}
        ...
        with tarfile.open(archive_path, "w:gz", compresslevel=3) as tar:
            def add_tree(source: Path, name: str, *, system: bool = False):
                if not source.exists():
                    return
                if not system and name.startswith("payload/pc_client/"):
                    actual_source = source.resolve()
                    # Only omit the development workstation's incidental state.
                    # A directory explicitly configured as SERVER media/workspace
                    # must remain portable, even if placed beneath pc_client.
                    configured = any(
                        actual_source == item or item in actual_source.parents
                        or (source.is_dir() and actual_source in item.parents)
                        for item in configured_paths
                    )
                    if not configured and is_agent_runtime_path(Path(name.removeprefix("payload/pc_client/")), directory=source.is_dir()):
                        return
            ...
            for field in PATH_FIELDS:
                ...
                if source.is_relative_to(root):
                    manifest["path_map"][field] = source.relative_to(root).as_posix()
                else:
    ...
    with tarfile.open(source, "r:gz") as tar:
        for member in tar:
            ...
            if (path.is_absolute() or ".." in path.parts or "\\" in member.name
                    ...
                    or not (member.isfile() or member.isdir()) or member.issparse() or str(path) in names):
...
def restore_snapshot(source: Path, destination: Path, *, password: str = "", private_key: bytes = b"",
    ...
    with tempfile.TemporaryDirectory(prefix=".xass-restore-", dir=destination.parent) as tmp:
        ...
        # Validate/serialize before any external DB side effects. Both workflows
        # share path relocation and cancellation of stale device commands.
        environment = _env_text(values)
        if manifest["database"] == "sqlite":
            shutil.copy2(unpacked / "database/sqlite.db", data / "serverredus.db")
            _prepare_sqlite(data / "serverredus.db", mapping)
        else:
            _restore_postgres(unpacked / "database/postgres.dump", values["database_url"], mapping)
        ...
        (payload / ".env").write_text(environment, encoding="utf-8")
