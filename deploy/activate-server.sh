...
public_config="$(cd "$root" && .venv/bin/python - <<'PY'
...
for key in ('profile_json_path', 'projects_json_path', 'quotes_json_path', 'site_config_json_path', 'profile_avatars_dir'):
    ...
    if not re.fullmatch(r'[A-Za-z0-9_./-]+', value):
        raise SystemExit('Custom public paths need manual nginx setup; see docs/SERVER_MIGRATION.md')
    print(value)
PY
)"
...
# The app reads .env itself; systemd EnvironmentFile would override the new
# restart manager below with the source machine's Docker/PM2 configuration.
...
server {
    ...
    location ~ /\. { deny all; }
    location ^~ /${public_paths[4]}/ { default_type application/octet-stream; add_header X-Content-Type-Options nosniff; try_files \$uri =404; }
    ...
    location ^~ /agent/ { proxy_pass http://127.0.0.1:8000; proxy_set_header Host \$host; proxy_set_header X-Forwarded-Proto \$scheme; }
    ...
    location ~ \.php$ {
        ...
        fastcgi_param PROFILE_JSON_PATH $root/${public_paths[0]};
        fastcgi_param PROJECTS_JSON_PATH $root/${public_paths[1]};
        fastcgi_param QUOTES_JSON_PATH $root/${public_paths[2]};
        fastcgi_param SITE_CONFIG_JSON_PATH $root/${public_paths[3]};
