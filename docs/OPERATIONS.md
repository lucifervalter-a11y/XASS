# XASS — operational reference

> Подробная техническая документация: установка, команды, переменные окружения и troubleshooting. Краткий обзор продукта находится в [главном README](../README.md).

Полный перенос с базой, медиа и ключами на другой VPS: [пошаговая миграция](MIGRATION.md). `deploy/backup.sh` теперь создаёт проверяемый полный архив вместо небезопасного копирования работающего SQLite-файла; `deploy/restore.sh` подготавливает новую установку без повторного ввода секретов.
...
- При `SAVE_FULL` тексты и метаданные индексируются на backend, а байты медиа сохраняются только выбранными Windows‑агентами.
- Экспорт логов в CSV.
...
- `/chats` - список сохранённых переписок.
- `/chat <chat_id>` - последние сообщения выбранной переписки.
- `/deleted` - последние удалённые сообщения.
- `/archive` - состояние локального архива; `/archive on <агент>` и `/archive off <агент>` выбирают ПК‑хранилища.
...
1. Откройте XASS внутри Telegram и перейдите в `Инструменты` → `iPhone и веб-приложение`.
2. Укажите публичный HTTPS-адрес сервера, например `https://redvps.site`, и нажмите `Создать ссылку для iPhone`.
3. XASS сохранит публичный адрес, попробует обновить кнопку Mini App у бота и выдаст одноразовую ссылку сроком на 10 минут.
4. Откройте эту ссылку в Safari. Код проверяется один раз, после чего удаляется из адресной строки, а браузер получает подписанную `HttpOnly`-сессию владельца.
5. Нажмите `Поделиться` → `На экран Домой`. Установленное PWA продолжит работать без повторного Telegram Login Widget.
6. В `Инструменты` → `Face ID и Passkey` зарегистрируйте устройство. После этого вход и подтверждение блокировки/перезапуска ПК будут использовать Face ID, Touch ID или Windows Hello.

Одноразовый код хранится в БД только как SHA-256-хэш, аннулируется после первого использования и не отправляется серверу в URL: он расположен после `#`, поэтому не попадает в HTTP-логи. Создание новой ссылки отзывает предыдущую.

`/setdomain` в BotFather остаётся дополнительной настройкой кнопки и резервного Telegram Login Widget, но больше не блокирует вход iPhone. В режиме разработки по обычному HTTP можно временно задать `PWA_COOKIE_SECURE=false`; публичный iPhone-сценарий принимает только HTTPS.

Добавляйте ключ и входите по одному и тому же адресу XASS. Если встроенный браузер Telegram не поддерживает Passkey, откройте одноразовую ссылку в Safari, Chrome или Edge и зарегистрируйте ключ там. При отмене проверки можно повторить добавление кнопкой; незавершённый ключ не сохраняется.

После загрузки новой версии PWA появится кнопка `Обновить приложение` в разделе `iPhone и веб-приложение`. Сначала сохраните изменения, затем нажмите кнопку. Автоматической перезагрузки посреди входа или работы с формой нет. Без интернета приложение показывает страницу отсутствия связи; для входа, данных и команд нужен сервер.

`Проверить обновление` в разделе сервера показывает ошибки Git отдельно от отсутствия обновлений. Обновление устанавливает проверенный commit из настроенной ветки только через fast-forward. Если локальная история разошлась с GitHub, сначала согласуйте изменения в репозитории; при недоступном GitHub установка не начинается.

## Архив сообщений на ПК

1. Установите агент версии `0.10.0` или новее.
2. В Windows‑приложении откройте `Настройки`, выберите папку локального архива и сохраните настройки.
3. В Mini App раскройте нужный агент и нажмите `Хранить архив здесь`, либо выполните `/archive on ИМЯ_АГЕНТА` в боте.
4. Агент синхронизирует события примерно раз в пять секунд. Тексты, версии и отметки удаления попадают в локальный `xass-archive.sqlite3`, файлы — в подпапку `media`.
5. Один и тот же архив можно включить сразу на нескольких агентах; каждый хранит независимую копию.

Backend не записывает новые медиафайлы на VPS: во время синхронизации он обновляет временный Telegram file path и передаёт поток авторизованному агенту. Если Telegram уже перестал отдавать файл, Mini App покажет метаданные, а сохранённая ранее локальная копия останется на ПК.

Ограничение Telegram: событие `deleted_business_messages` доступно только для Business‑диалогов, которые Telegram передал подключённому бизнес‑боту. Удаления произвольных сообщений обычный Bot API не раскрывает, поэтому обещать перехват «всего Telegram» технически невозможно.
