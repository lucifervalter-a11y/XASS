...
class PortableMigrationTests(unittest.TestCase):
    ...
    def test_cli_export_inspect_restore_on_fixtures(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            script = Path(__file__).resolve().parents[1] / "deploy" / "portable_migrate.py"
            commands = [
    ...
    def test_only_checkout_pc_state_is_excluded_and_server_state_is_preserved(self):
        with tempfile.TemporaryDirectory() as temporary:
            directory = Path(temporary)
            root, settings = self.fixture(directory)
            ...
            for name in names:
                path = root / "pc_client" / name
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(workstation_secret)
            (root / "pc_client" / "secret_store.py").write_text("# keep source")
            (root / "data" / ".xass-master.key").write_bytes(b"SERVER_KEY_MUST_SURVIVE")
            (root / "data" / "config.json.bak").write_bytes(b"SERVER_BACKUP_MUST_SURVIVE")
            # Preserve an explicitly configured server folder even under pc_client.
            workspace = root / "pc_client" / "Archive" / "server-workspace"
            workspace.mkdir()
            (workspace / "upload.txt").write_bytes(b"SERVER_UPLOAD_MUST_SURVIVE")
            settings["agent_workspace_dir"] = str(workspace)
            archive = directory / "private.tar.gz"
            export_server_archive(root, archive, settings)
            with tarfile.open(archive) as packaged:
                members = set(packaged.getnames())
                for name in names:
                    self.assertNotIn("pc_client/" + name, members)
                self.assertIn("pc_client/secret_store.py", members)
                self.assertEqual(packaged.extractfile("data/.xass-master.key").read(), b"SERVER_KEY_MUST_SURVIVE")
                self.assertEqual(packaged.extractfile("data/config.json.bak").read(), b"SERVER_BACKUP_MUST_SURVIVE")
                self.assertEqual(packaged.extractfile("pc_client/Archive/server-workspace/upload.txt").read(), b"SERVER_UPLOAD_MUST_SURVIVE")
                self.assertIn(b"original-session-secret", packaged.extractfile(".env").read())
